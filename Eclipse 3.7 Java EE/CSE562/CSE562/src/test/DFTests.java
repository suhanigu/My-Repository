...
public class DFTests {
	...
	static{
		...
		System.out.println("datafile restored " + df.getRecord(0));
	}
	...
	public void InsRec(){
		...
		try{
			...
			//System.out.println("First record of movieVault file " + df.getRecord(0));
			int id = df1.insertRecord(df.getRecord(0));
			System.out.println("Inserting record id: " + id);
	...
	public void iterator(){
		...
		if(eCaught){
			...
			try{
				...
				System.out.println("count: " + count + "rs1 size: " + rs1.size());
				if(count == rs1.size()){
